...
export default class GeneralController {
	static async registerAccount(req: Request, res: Response): Promise<Response> {
		...
		const filterData = new GetUserDTO(email)

		const existUser = await UserSchema.findOne(filterData.getByField()).select(
		...
		const user = new UserSchema(new RegisterDTO(name, email, phone, encryptedPassword))
	...
	static async login(req: Request, res: Response): Promise<Response> {
		...
		const filterData = new GetUserDTO(email)
		const user = await UserSchema.findOne(filterData.getByField())
	...
	static async checkLoggedUser(req: Request, res: Response): Promise<Response> {
		...
		const databaseUser = await UserSchema.findById(loggedUser._id).select(
			GetUserDTO.selectFieldsQuery()
		)
