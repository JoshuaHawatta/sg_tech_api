...
export default class JwtTokenHandler {
	static generateToken(client: TClient, res: Response) {
	...
	static async getClientByToken(req: Request, res: Response) {
		const token = await JwtTokenHandler.getToken(req, res)

		return jwt.verify(token, process.env.API_SECRET as string) as TClient
	}
